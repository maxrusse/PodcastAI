#!/usr/bin/env python3
"""PodcastAI Gemini pipeline (first full version, 2026 baseline)."""
...
def gemini_text_json(client: genai.Client, prompt: str) -> dict[str, Any]:
...
def gemini_generate_png(client: genai.Client, prompt: str, out_path: Path) -> None:
...
def build_course_plan(client: genai.Client, transcript: str) -> dict[str, Any]:
    prompt = (
    ...
    plan = gemini_text_json(client, prompt)
    if "sections" not in plan:
        raise ValueError("Model response missing 'sections'.")
    return plan


def build_after_episode_package(client: genai.Client, transcript: str, course_plan: dict[str, Any]) -> dict[str, Any]:
    prompt = (
    ...
    return gemini_text_json(client, prompt)
...
def main() -> None:
    ...
    for section in course_plan.get("sections", []):
        ...
        if MAKE_SECTION_IMAGES and section.get("image_prompt"):
            image_path = SECTIONS_DIR / f"{path.stem}_gemini.png"
            gemini_generate_png(client, section["image_prompt"], image_path)
    ...
    if MAKE_STATIC_MATERIAL_IMAGES:
        ...
        for idx, material in enumerate(materials, start=1):
            if isinstance(material, dict) and material.get("image_prompt"):
                ...
                gemini_generate_png(client, material["image_prompt"], path)
    ...
    print("Done. Output in out_google/")
